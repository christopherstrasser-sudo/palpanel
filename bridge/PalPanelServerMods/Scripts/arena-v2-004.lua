-- PalPanelServerMods Arena v2 step 4 / v0.3.0
-- SERVER-ONLY / NO CLIENT MOD.
--
-- Arena activation is gated by raid-arena-trigger.txt. Nothing is moved and no
-- keep-in is enforced before the first verified hit on the exact raid boss.
-- Once triggered, online players are placed on a compact safe ring well inside
-- the enlarged barrier. This avoids the previous 1750-unit placement intersecting
-- the scaled barrier shell and pinning the player in collision.
...
local PLAYER_SPAWN_RADIUS = 900.0
local PLAYER_Z_OFFSET = 100.0
        ...
        if valid(ps) then
            ...
            if valid(pawn) then
                ...
                if key ~= "" and not seen[key] then
                    ...
                    if at then
                        seen[key] = true
                        rows[#rows + 1] = { key = key, uid = uid, name = name, pawn = pawn, at = at }
                    end
                end
            end
        end
...
local function writeStatus()
    ...
    local lines = {
        ...
        "backend=first_hit_gated_boss_centered_arena",
        "world_mutation=raid_player_teleport_after_first_hit_only",
        "actor_spawn=0",
        "npc_spawn=0",
        "boss_mutation=0",
...
local function placeInitialPlayers()
    ...
    placementDone = true
    log(string.format("first-hit arena entry complete: %d/%d player(s)", placements, count))
end
    ...
    for _, player in ipairs(onlinePlayers()) do
        if isTracked(player) then
            ...
            if dist > RADIUS then
                ...
                if teleportPlayer(player, target, "keep-in") then teleports = teleports + 1
                else teleportFailures = teleportFailures + 1 end
            end
        end
    end
end
...
local function tick()
    ...
    if active and triggerMatches and not arenaTriggered then
        ...
        log(string.format("first-hit arena activated: %.1f %.1f %.1f", center.X, center.Y, center.Z))
    end

    if active and arenaTriggered then
        ExecuteInGameThread(function()
            local ok, err = xpcall(function() enforceKeepIn(raid) end, debug.traceback)
            if not ok then lastError = tostring(err); log("arena tick failed: " .. lastError) end
        end)
...
local function schedule()
    ...
    local function loop()
        ...
        if not ok then lastError = tostring(err); log("step 4 tick failed: " .. lastError) end
...
writeStatus()
schedule()
log("v" .. VERSION .. " loaded; waiting for first-hit arena trigger")
