-- PalPanelServerMods Arena v2 step 5 / v0.4.0
-- SERVER-ONLY / NO CLIENT MOD.
--
-- Nothing happens before the first verified boss hit. When triggered, players are
-- placed once on a very small safe ring around the live boss-centered arena point.
-- Keep-in remains server authoritative. The visual barrier is handled separately.
...
local PLAYER_SPAWN_RADIUS = 450.0
local PLAYER_Z_OFFSET = 150.0
...
local function writeStatus()
    ...
    local lines = {
        ...
        "backend=first_hit_live_boss_center_safe_player_ring",
...
local function placeInitialPlayers()
    ...
    placementDone = true
    log(string.format("first-hit arena entry complete: %d/%d player(s), safe radius %.0f", placements, count, PLAYER_SPAWN_RADIUS))
end
...
local function schedule()
    ...
    local function loop()
        ...
        if not ok then
            lastError = tostring(err)
            log("step 5 tick failed: " .. lastError)
        end
...
writeStatus()
schedule()
log("v" .. VERSION .. " loaded; waiting for first-hit live-boss arena trigger")
