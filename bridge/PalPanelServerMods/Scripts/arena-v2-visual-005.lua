-- PalPanelServerMods Arena v2 step 4 / visual v0.5.0
-- SERVER-ONLY / NO CLIENT MOD.
--
-- The proven persistent BP_LevelObject_TowerLockBarrier_C is now gated by the
-- first-hit trigger. Before the exact raid boss is hit there is no barrier.
-- After the trigger, one barrier is spawned at the arena center, scaled larger,
-- and bLocked=true is replicated exactly once.
--
-- No repeated component visibility writes, no Niagara, no collision mutation,
-- no UObject scan and no custom RPCs.
...
local function writeStatus()
    local lines = {
        ...
        "backend=first_hit_gated_enlarged_tower_lock_barrier",
...
local function spawnBarrier(trigger)
    ...
    observeBarrier()
    visualStatus = "first_hit_barrier_active"
    writeStatus()
    log(string.format("first-hit barrier active at %.0f %.0f %.0f; scale %.1fx%.1fx%.1f", barrierX, barrierY, barrierZ, SCALE_XY, SCALE_XY, SCALE_Z))
end
...
local function tick()
    ...
    if raidId ~= currentRaidId then
        if valid(barrier) then ExecuteInGameThread(function() cleanupBarrier() end) end
        resetRaid(raidId)
    end
    ...
    if active and triggerMatches then
        ...
        elseif valid(barrier) and triggerTicks % OBSERVE_EVERY_TICKS == 0 then
            ExecuteInGameThread(function()
                ...
                if not callOk then lastError = tostring(callErr); log("barrier observation failed: " .. lastError) end
                writeStatus()
            end)
        end
    elseif not active and valid(barrier) then
...
local function schedule()
    ...
    local function loop()
        ...
        if not ok then lastError = tostring(err); log("visual tick failed: " .. lastError) end
...
writeStatus()
schedule()
log("v" .. VERSION .. " loaded; barrier waits for first-hit trigger")
