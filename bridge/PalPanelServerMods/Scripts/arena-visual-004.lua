-- PalPanelServerMods raid arena visual v0.4.0
--
-- SERVER-ONLY / NO CLIENT MOD.
-- Uses the already-loaded vanilla brazier AStaticMeshActor class as a visible
-- arena marker, but unlike v0.3.0 it temporarily enables replication on the
-- CLASS DEFAULT OBJECT before each spawn batch. This makes the actors enter the
-- world as replicated actors instead of switching replication on only after
-- SpawnActor returned.
--
-- Stability rules:
--   * NO PalBuildObject
...
--   * NO AssetRegistry scan
--   * only exact already-loaded BP class resolution
--   * small spawn batches on the game thread
--
...
local cdoReplicationApplied = 0
local actorReplicationBoosted = 0
...
local lastActive = {
    ...
    resolvedClass = "",
    error = "",
    cdoReplicationApplied = 0,
    actorReplicationBoosted = 0,
    centerX = 0,
    centerY = 0,
    centerZ = 0,
    visualZ = 0
...
local function snapshotActive()
    ...
    lastActive.cdoReplicationApplied = cdoReplicationApplied
    lastActive.actorReplicationBoosted = actorReplicationBoosted
...
local function writeStatus(raid)
    ...
    local lines = {
        ...
        "backend=born_replicating_vanilla_static_mesh",
        ...
        "cdo_replication_applied=" .. tostring(cdoReplicationApplied),
        "actor_replication_boosted=" .. tostring(actorReplicationBoosted),
        ...
        "last_active_cdo_replication_applied=" .. tostring(lastActive.cdoReplicationApplied),
        "last_active_actor_replication_boosted=" .. tostring(lastActive.actorReplicationBoosted),
...
local function resetLastActive()
    ...
    lastActive.error = ""
    lastActive.cdoReplicationApplied = 0
    lastActive.actorReplicationBoosted = 0
...
local function readProperty(obj, name)
    if not valid(obj) then return false, nil end
    local ok, value = pcall(function() return obj[name] end)
    if not ok then return false, nil end
    return true, unwrap(value)
end

local function withBornReplication(class, fn)
    local cdo = getClassDefaultObject(class)
    if not cdo then
        return fn(), false
    end
    ...
    for _, name in ipairs(names) do
        local ok, value = readProperty(cdo, name)
        originals[name] = { ok = ok, value = value }
    end

    local applied = 0
    if setProperty(cdo, "bReplicates", true) then applied = applied + 1 end
    if setProperty(cdo, "bReplicateMovement", true) then applied = applied + 1 end
    if setProperty(cdo, "bAlwaysRelevant", true) then applied = applied + 1 end
    if setProperty(cdo, "bNetLoadOnClient", true) then applied = applied + 1 end
    if setProperty(cdo, "NetCullDistanceSquared", NET_CULL_DISTANCE_SQUARED) then applied = applied + 1 end

    if applied > cdoReplicationApplied then cdoReplicationApplied = applied end

    local results = { fn() }

    for _, name in ipairs(names) do
        local original = originals[name]
        if original and original.ok then
            pcall(function() cdo[name] = original.value end)
        end
    end

    return table.unpack(results), applied > 0
end

local function boostActorReplication(actor)
...
local function beginVisual(raid)
    ...
    if not valid(worldFromPlayers(players)) then
        ...
        lastError = "UWorld unavailable for server-only visual ring"
        snapshotActive()
        writeStatus(raid)
        return false
    end
    ...
    nextIndex = 0
    visualStatus = "spawning_born_replicating"
    lastError = ""
    snapshotActive()
    log(string.format("born-replicating FIRE arena start: radius=%d markers=%d class=%s", math.floor(RADIUS), MARKERS, VISUAL_CLASS_NAME))
    writeStatus(raid)
    return true
end
...
local function spawnBatch(raid)
    ...
    if not valid(world) then
        ...
        lastError = "UWorld unavailable while spawning fire ring"
        snapshotActive()
        writeStatus(raid)
        return
    end
    ...
    while nextIndex < MARKERS and spawnedThisBatch < SPAWN_BATCH do
        ...
        local ok, result = withBornReplication(class, function()
            return pcall(function()
                return world:SpawnActor(class, location, rotation)
            end)
        end)

        -- withBornReplication returns the values produced by fn(). Here those
        -- values are pcall(success, SpawnActorResult).
        local spawnOk = ok
        local actor = unwrap(result)

        if spawnOk and valid(actor) then
            local boosted = boostActorReplication(actor)
            actorReplicationBoosted = actorReplicationBoosted + boosted
    ...
    if nextIndex >= MARKERS then
        if #actors == MARKERS then
            visualStatus = "server_fire_ring_born_replicating"
            lastError = ""
        ...
        log(string.format("fire arena spawn finished: %d/%d; cdo_flags=%d actor_flags=%d", #actors, MARKERS, cdoReplicationApplied, actorReplicationBoosted))
    else
        visualStatus = "spawning_born_replicating"
    end
...
log("v" .. VERSION .. " loaded; born-replicating SERVER-ONLY vanilla fire-ring backend")
log("No BuildObject, no SkillEffect, no AssetRegistry scan, no client mod")
